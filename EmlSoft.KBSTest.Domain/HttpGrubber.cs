    ...
    public class HttpGrubber : IGrubber
    {
        ...
        public static string StripHTML(string source)
        {
            try
            {
                ...
                var qq = System.Text.RegularExpressions.Regex.Matches(result, @"(<script>).*?(</script>)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                
                foreach(Match m in qq )
                {
                    if (m.Value.IndexOf("УрГПУ") > 0)
                    {
                        string a = m.Value;
                    }
                }
